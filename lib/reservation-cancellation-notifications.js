 ...
 */
export async function notifyBookingCancelled({ reservation, booking, db = pool, audit = {} }) {
  ...
  if (customerEmail && !sameRecipient) {
    notifySubmitter({
      ...
      cancelled: true,
      timezone,
      audit: { kind: "cancellation", entityType: "booking", entityId: bookingId, projectId, actor: { actor_type: "system" }, ...audit },
  ...
  try {
    await createNotification({
      ...
      audit: { entityLabel: `booking #${bookingId}`, projectId, actor: { actor_type: "system" }, ...audit },
    });
  } catch (err) {
  ...
  if (workerPushEnabled) {
    try {
      await sendPushToUser(workerId, {
        ...
        url: "/",
      }, { entityType: "booking", entityId: bookingId, entityLabel: `booking #${bookingId}`, projectId, actor: { actor_type: "system" }, ...audit });
    } catch (err) {
