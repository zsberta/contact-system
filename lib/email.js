...
import {
  ...
  renderBookingDeletedNotification,
  renderWorkerBookingNotification,
...
function resolveFrom() {
  ...
  // operator notification can say "Nexus" while the
...
// "Nexus". Pass a different value when the email should
// look like it came from somewhere specific — e.g. the project name
// for a submitter auto-reply.
export async function sendMail({ to, subject, text, html, fromName, fromAddress }) {
  // Global kill switch — set EMAIL_SENDING=false in .env to disable ALL
  // outgoing email (booking notifications, invites, password resets, the
  // queue worker). Everything funnels through sendMail, so this is the
  // single gate. Anything else (true/unset) sends normally.
  ...
  try {
    ...
    const info = await transport.sendMail({
      // fromAddress lets a caller override the envelope From email (e.g.
      // the assigned worker's own address on booking confirmations).
      // Anything without an @ falls back to the default sender.
      from: `"${displayName}" <${(typeof fromAddress === "string" && fromAddress.includes("@")) ? fromAddress.trim() : resolveFrom()}>`,
      to,
      subject,
      text,
      html: html || text,
...
export async function notifyProjectOwner({
  ...
  startsAt,
  endsAt,
  // New structured booking context (optional, for enriched notifications)
  serviceName,
  customerName,
  customerEmail,
  customerPhone,
  // Optional override for the recipient address (e.g. worker email)
  to: toOverride,
  // Optional audit sidecar for the activity log: { action?, kind?,
  // entityType?, entityId?, entityLabel?, projectId?, actor? }.
  // Forwarded to enqueueMail; runJob logs one email.send SEND row.
  audit = {},
}) {
  try {
    ...
    const to = (typeof toOverride === "string" && toOverride.trim().length > 0)
      ? toOverride.trim()
      : (typeof project.customer_email === "string"
    ...
    // Build enriched data: prefer structured booking fields, fall back to raw data
    const enrichedData = (serviceName || customerName)
      ? {
    ...
    // From: "Nexus" <envelope> — always HU, always the
    enqueueMail({
      to,
      subject,
      html,
      text,
      fromName: "Nexus",
      audit: { kind: "form_notification", entityType: "form", projectId, actor: { actor_type: "system" }, ...audit },
    });
    return true;
  } catch (err) {
...
// brand ("Nexus") to the visitor — multi-tenant plumbing stays
// invisible.
export async function notifySubmitter({
  ...
  bookingId,
  serviceName,
  // New: direct email from structured booking data (preferred over data detection)
  email: directEmail,
  // Optional override for the recipient address (e.g. worker email)
  to: toOverride,
  // Optional override for the sign-off name (e.g. worker name for customer email)
  signerName,
  // When true, skip project data lookup and use Nexus branding (for worker notifications)
  useBrandDefaults,
  // When true, remove reply hint and sign-off from the email
  hideReplyHint,
  hideSignOff,
  // Customer details for worker notification
  customerName,
  customerEmail: customerEmailAddress,
  customerPhone,
  comment,
  // Booking self-service: opaque token + reservation secret for manage page links
  bookingToken,
  secretToken,
  // Reservation timezone for email date formatting
  timezone,
  // Worker cancellation emails use the operator-facing worker template.
  cancelled,
  // Optional audit sidecar for the activity log (see notifyProjectOwner).
  audit = {},
}) {
  try {
    // Prefer structured booking email over data-bag detection
    const submitterEmail = toOverride || directEmail || detectSubmitterEmail(data);
    ...
    // Resolve the assigned worker for booking emails so the customer
    // confirmation can display "Service Name - Worker Name" in the
    // service detail and send from the worker's own From address.
    // Only for booking auto-replies (never for form auto-replies or
    // the Nexus-branded worker notifications).
    let workerEmailAddress = "";
    let workerDisplayName = "";
    if (
      !useBrandDefaults
      && (kind === "reservation" || kind === "reservation_deleted")
      && bookingId
    ) {
      try {
        const wres = await pool.query(
          `SELECT u.first_name, u.last_name, u.email
           ...
           LEFT JOIN reservation_services s ON s.id = b.service_id
           LEFT JOIN users u ON u.id = COALESCE(b.worker_user_id, s.worker_user_id)
           WHERE b.id = $1`,
          [bookingId],
        );
        if (wres.rowCount > 0 && wres.rows[0].email) {
          const w = wres.rows[0];
          workerEmailAddress = w.email;
          workerDisplayName = locale === "hu"
            ? `${w.last_name || ""} ${w.first_name || ""}`.trim()
            : `${w.first_name || ""} ${w.last_name || ""}`.trim();
        }
      } catch (err) {
        console.error("[email/worker-lookup] failed:", err.code || "", err.message);
      }
    }

    let subject, html, text;
    if (kind === "reservation" && useBrandDefaults) {
      ({ subject, html, text } = renderWorkerBookingNotification({
        serviceName: serviceName || formName,
        customerName,
        customerEmail: customerEmailAddress,
        customerPhone,
        comment,
    ...
    } else if (kind === "reservation_deleted") {
      ({ subject, html, text } = renderBookingDeletedNotification({
        projectName,
        domainAddress,
        customerEmail,
        startsAt,
        ...
        timezone,
        signerName,
        workerName: workerDisplayName || undefined,
      }));
    } else if (kind === "reservation") {
      const appUrl = (process.env.APP_PUBLIC_URL || "").replace(/\/+$/, "");
      const embedBaseUrl = (secretToken && appUrl)
    ...
    // never see "Nexus" anywhere. When the project has
    ...
    // Default SEND linkage derived from args the helper already receives;
    // an explicit caller `audit` wins via spread-last.
    const derivedKind = cancelled || kind === "reservation_deleted"
      ? "cancellation"
      : kind === "reservation"
        ? "confirmation"
        : "form_autoreply";
    enqueueMail({
      ...
      text,
      fromName,
      // From the worker's own address when one is assigned to the
      // booking's service; otherwise the project/default sender.
      fromAddress: workerEmailAddress || undefined,
      audit: {
        kind: derivedKind,
...
// ---------------------------------------------------------------------------
// notifyWorker — optional notification to the assigned worker's user email.
// Best-effort: failure never rolls back a committed booking.
// ---------------------------------------------------------------------------

export async function notifyWorker({ workerUserId, serviceName, customerName, startsAt, endsAt, locale, audit = {} }) {
  if (!workerUserId) return false;
  try {
    const { rows } = await pool.query(
      `SELECT first_name, last_name, email FROM users WHERE id = $1 AND role = 'enduser'`,
      [workerUserId],
    );
    if (rows.length === 0) return false;
    const worker = rows[0];
    const to = typeof worker.email === "string" ? worker.email.trim() : "";
    if (to.length === 0) return false;

    const workerName = `${worker.last_name || ""} ${worker.first_name || ""}`.trim() || "Worker";
    const subject = `Új foglalás — ${serviceName || "Szolgáltatás"}`;
    const text = `Kedves ${workerName}!\n\nÚj foglalás érkezett a ${serviceName || "szolgáltatás"} szolgáltatásra.\nÜgyfél: ${customerName || "N/A"}\nIdőpont: ${startsAt || ""} – ${endsAt || ""}\n\nÜdvözlettel,\nNexus`;
    const html = `<p>Kedves ${workerName}!</p><p>Új foglalás érkezett a <strong>${serviceName || "szolgáltatás"}</strong> szolgáltatásra.</p><p>Ügyfél: ${customerName || "N/A"}<br>Időpont: ${startsAt || ""} – ${endsAt || ""}</p><p>Üdvözlettel,<br>Nexus</p>`;

    enqueueMail({ to, subject, html, text, fromName: "Nexus", audit: { kind: "confirmation", entityType: "booking", actor: { actor_type: "system" }, ...audit } });
    return true;
  } catch (err) {
    console.error("[email/worker-notify] failed:", err.code || "", err.message);
    return false;
  }
}
