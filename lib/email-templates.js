...
// "Nexus" the central admin tool. The CRM is multi-tenant; the
...
export const BRAND = {
  ...
  productName: { hu: "Nexus", en: "Nexus" },
  logoUrl: "https://nexus.zsoltberta.hu/logo.svg",
  siteUrl: "https://nexus.zsoltberta.hu",
...
// "Nexus" anywhere the visitor can see.
...
const KNOWN_FIELDS = {
  ...
  service: {
    hu: "Szolgáltatás",
    en: "Service",
    aliases: ["service", "szolgaltatas", "servicename"],
  },
  worker: {
    hu: "Felelős",
    en: "Worker",
    aliases: ["worker", "felelos", "workername", "assignedto"],
  },
};
...
// ---------------------------------------------------------------------------
//
// `header` overrides the default "Nexus" brand word + link with
// project-specific values, so submitter-facing
...
// brand-level product name ("Nexus") is intentionally absent everywhere
// in this template.
//
...
export function renderBookingAutoReply({
  ...
  bookingId,
  serviceName,
  workerName, // optional — renders the service as "Service Name - Worker Name"
  locale: rawLocale,
  timezone: rawTimezone, // reservation timezone for date formatting (e.g. "Europe/Budapest")
  signerName, // optional override for the sign-off name
  hideReplyHint, // remove "don't reply" text
  hideSignOff, // remove sign-off
  bookingToken, // opaque token for customer self-service manage page
  embedBaseUrl, // base URL for the embed page (e.g. https://app.example.com/embed/reservations/)
  cancelled = false,
}) {
  const locale = normaliseLocale(rawLocale);
  const header = resolveProjectIdentity({
  ...
  // Format date/time for the email body — use reservation timezone
  const tz = rawTimezone || "UTC";
  const fmtDate = (iso) => {
    if (!iso) return "";
    const d = new Date(iso);
    return locale === "hu"
      ? d.toLocaleDateString("hu-HU", { year: "numeric", month: "long", day: "numeric", timeZone: tz })
      : d.toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric", timeZone: tz });
  };
  const fmtTime = (iso) => {
    if (!iso) return "";
    const d = new Date(iso);
    return d.toLocaleTimeString(locale === "hu" ? "hu-HU" : "en-US", { hour: "2-digit", minute: "2-digit", timeZone: tz });
  };

  const confirmed = cancelled
    ? (locale === "hu" ? "Foglalás lemondva" : "Booking cancelled")
    : (locale === "hu" ? "Foglalás megerősítve!" : "Booking confirmed!");
  const idLabel = locale === "hu" ? "Foglalás azonosító" : "Booking ID";
  const serviceLabel = locale === "hu" ? "Szolgáltatás" : "Service";
  const dateLabel = locale === "hu" ? "Időpont" : "Date & time";

  const dateRange = startsAt && endsAt
    ? `${fmtDate(startsAt)}, ${fmtTime(startsAt)} – ${fmtTime(endsAt)}`
    : "";

  // Worker display: the service renders as "Service Name - Worker Name"
  // (Hungarian name order: last first) when a worker is assigned.
  const serviceDisplay = serviceName && workerName
    ? `${serviceName} - ${workerName}`
    : serviceName;

  // Subject datetime in the reservation timezone, numeric style
  // (hu: "2026. 09. 10. 10:00"). Status word: Megerősítve / Confirmed.
  const subjectDate = startsAt
    ? new Date(startsAt).toLocaleString(locale === "hu" ? "hu-HU" : "en-US", {
        year: "numeric",
        month: "2-digit",
        day: "2-digit",
        hour: "2-digit",
  ...
  const statusWord = cancelled
    ? (locale === "hu" ? "Lemondva" : "Cancelled")
    : (locale === "hu" ? "Megerősítve" : "Confirmed");
  ...
  const subject = serviceName
    ? `${serviceName}${subjectDate ? ` - ${subjectDate}` : ""} - ${statusWord}`
    : (cancelled ? (locale === "hu" ? "Foglalás lemondva" : "Booking cancelled") : (locale === "hu" ? "Foglalás megerősítve" : "Booking confirmed"));

  const detailRows = [
    serviceName ? `<tr><td style="padding:6px 0;font-size:13px;color:${BRAND.muted};">${esc(serviceLabel)}:</td><td style="padding:6px 0 6px 12px;font-size:13px;color:${BRAND.foreground};font-weight:600;">${esc(serviceDisplay)}</td></tr>` : "",
    dateRange ? `<tr><td style="padding:6px 0;font-size:13px;color:${BRAND.muted};">${esc(dateLabel)}:</td><td style="padding:6px 0 6px 12px;font-size:13px;color:${BRAND.foreground};font-weight:600;">${esc(dateRange)}</td></tr>` : "",
    bookingId ? `<tr><td style="padding:6px 0;font-size:13px;color:${BRAND.muted};">${esc(idLabel)}:</td><td style="padding:6px 0 6px 12px;font-size:13px;color:${BRAND.foreground};font-weight:600;">#${esc(String(bookingId))}</td></tr>` : "",
  ].filter(Boolean).join("");

  const bodyHtml = `<h1 style="margin:0 0 16px 0;font-size:24px;line-height:1.25;font-weight:700;letter-spacing:-0.015em;color:${BRAND.foreground};">${esc(confirmed)}</h1>
${detailRows ? `<table role="presentation" cellpadding="0" cellspacing="0" border="0" style="margin:0 0 24px 0;border-collapse:collapse;">${detailRows}</table>` : ""}
${(() => {
    const manageUrl = (bookingToken && embedBaseUrl)
      ? `${embedBaseUrl.replace(/\/+$/, "")}/manage/${bookingToken}`
      : null;
    if (!manageUrl) return "";
    const modifyLabel = locale === "hu" ? "Módosítás" : "Modify booking";
    const deleteLabel = locale === "hu" ? "Lemondás" : "Cancel booking";
    return `
<table role="presentation" cellpadding="0" cellspacing="0" border="0" style="margin:0 0 24px 0;">
  <tr>
    <td style="padding:0 12px 12px 0;">
      <a href="${esc(manageUrl)}" style="display:inline-block;padding:10px 20px;background:${BRAND.primaryAccent};color:#ffffff;font-size:14px;font-weight:600;text-decoration:none;border-radius:6px;">${esc(modifyLabel)}</a>
    </td>
    <td style="padding:0 0 12px 0;">
      <a href="${esc(manageUrl)}" style="display:inline-block;padding:10px 20px;background:#ffffff;color:${BRAND.primaryAccent};font-size:14px;font-weight:600;text-decoration:none;border-radius:6px;border:1px solid ${BRAND.border};">${esc(deleteLabel)}</a>
    </td>
  </tr>
</table>`;
  })()}
${hideReplyHint ? "" : `<p style="margin:0 0 24px 0;font-size:14px;line-height:1.6;color:${BRAND.muted};">${replyHint}</p>`}
${hideSignOff ? "" : `<p style="margin:24px 0 0 0;font-size:14px;line-height:1.5;color:${BRAND.muted};">${signOff}<br><strong style="color:${BRAND.foreground};">${esc(signatureName)}</strong></p>`}`;
  ...
  const detailText = [
    serviceName ? `${serviceLabel}: ${serviceDisplay}` : "",
    dateRange ? `${dateLabel}: ${dateRange}` : "",
    bookingId ? `${idLabel}: #${bookingId}` : "",
  ].filter(Boolean).join("\n");

  const replyText = hideReplyHint ? "" : (locale === "hu"
  ...
  const signOffText = hideSignOff ? "" : `\n${signOff}\n${signatureName}`;

  const text = `${confirmed}
${detailText ? "\n" + detailText : ""}${replyText}${signOffText}`;

  return { subject, html, text };
}

// ---------------------------------------------------------------------------
// Template 2b-reminder: booking reminder
//
// Sent to the customer X hours before the booking starts. Mirrors the
// confirmation layout but with reminder-specific wording.
// ---------------------------------------------------------------------------
export function renderBookingReminder({
  ...
  const locale = normaliseLocale(rawLocale);
  const header = resolveProjectIdentity({ projectName, domainAddress, customerEmail });
  const tz = rawTimezone || "UTC";
  const fmtDate = (iso) => {
    if (!iso) return "";
    const d = new Date(iso);
  ...
  const reminder = locale === "hu" ? "Emlékeztető" : "Reminder";
  const intro = locale === "hu"
    ? `Ez egy emlékeztető, hogy a foglalásod ${hoursBefore} órán belül kezdődik.`
    : `This is a reminder that your booking starts in ${hoursBefore} hours.`;
  const idLabel = locale === "hu" ? "Foglalás azonosító" : "Booking ID";
  const serviceLabel = locale === "hu" ? "Szolgáltatás" : "Service";
  const dateLabel = locale === "hu" ? "Időpont" : "Date & time";
  const dateRange = startsAt && endsAt
  ...
  const subjectDate = startsAt
    ...
    : "";
  const subject = serviceName
    ? `${serviceName}${subjectDate ? ` - ${subjectDate}` : ""} - ${reminder}`
    : (locale === "hu" ? "Foglalási emlékeztető" : "Booking reminder");

  const replyHint = locale === "hu"
    ? `<strong>Erre az e-mailre ne válaszolj.</strong> Ha bármit módosítanod kell, írj nekünk közvetlenül ide: <a href="mailto:${header.writeTo}" style="color:${BRAND.primaryAccent};text-decoration:underline;font-weight:600;">${header.writeTo}</a>.`
    : `<strong>Please do not reply to this email.</strong> If you need to change anything, write to us directly at <a href="mailto:${header.writeTo}" style="color:${BRAND.primaryAccent};text-decoration:underline;font-weight:600;">${header.writeTo}</a>.`;
  const signOff = locale === "hu" ? "Üdvözlettel," : "Best regards,";
  const signatureName = signerName || header.name || (locale === "hu" ? "a csapat" : "the team");
  ...
  const bodyHtml = `<h1 style="margin:0 0 16px 0;font-size:24px;line-height:1.25;font-weight:700;letter-spacing:-0.015em;color:${BRAND.foreground};">${esc(reminder)}</h1>
<p style="margin:0 0 24px 0;font-size:15px;line-height:1.6;color:${BRAND.muted};">${esc(intro)}</p>
${detailRows ? `<table role="presentation" cellpadding="0" cellspacing="0" border="0" style="margin:0 0 24px 0;border-collapse:collapse;">${detailRows}</table>` : ""}
${(() => {
    const manageUrl = (bookingToken && embedBaseUrl)
      ? `${embedBaseUrl.replace(/\/+$/, "")}/manage/${bookingToken}`
...
<p style="margin:24px 0 0 0;font-size:14px;line-height:1.5;color:${BRAND.muted};">${signOff}<br><strong style="color:${BRAND.foreground};">${esc(signatureName)}</strong></p>`;
  ...
  const html = layout({ locale, preheader: intro, bodyHtml, header });
...
// ---------------------------------------------------------------------------
// Template 2c: booking deleted notification
//
// Sent to the customer when an operator deletes their booking. Same layout
// and detail formatting as the confirmation, but with "deleted" wording.
// ---------------------------------------------------------------------------

export function renderBookingDeletedNotification({
  projectName,
  domainAddress,
  customerEmail,
  startsAt,
  ...
  const deleted = locale === "hu" ? "Foglalás törölve" : "Booking deleted";
  const idLabel = locale === "hu" ? "Foglalás azonosító" : "Booking ID";
  const serviceLabel = locale === "hu" ? "Szolgáltatás" : "Service";
  const dateLabel = locale === "hu" ? "Időpont" : "Date & time";
  ...
  // Worker display + subject parts — same shape as the confirmation email
  // so deleted bookings read "Service Name - Worker Name" and the subject
  // carries "Service Name - Date time - Törölve".
  const serviceDisplay = serviceName && workerName
    ? `${serviceName} - ${workerName}`
    : serviceName;
  const subjectDate = startsAt
    ...
    : "";
  const statusWord = locale === "hu" ? "Törölve" : "Deleted";
  ...
  const subject = serviceName
    ? `${serviceName}${subjectDate ? ` - ${subjectDate}` : ""} - ${statusWord}`
    : (locale === "hu" ? "Foglalás törölve" : "Booking deleted");
  ...
  const bodyHtml = `<h1 style="margin:0 0 16px 0;font-size:24px;line-height:1.25;font-weight:700;letter-spacing:-0.015em;color:${BRAND.foreground};">${esc(deleted)}</h1>
${detailRows ? `<table role="presentation" cellpadding="0" cellspacing="0" border="0" style="margin:0 0 24px 0;border-collapse:collapse;">${detailRows}</table>` : ""}
<p style="margin:24px 0 0 0;font-size:14px;line-height:1.5;color:${BRAND.muted};">${signOff}<br><strong style="color:${BRAND.foreground};">${esc(signatureName)}</strong></p>`;
  ...
  const text = `${deleted}
${detailText ? "\n" + detailText : ""}${signOffText}`;

  return { subject, html, text };
}

// ---------------------------------------------------------------------------
// Template 2b: worker booking notification
//
// Sent to the assigned worker when a new booking is created. Uses Nexus
// branding (operator-facing). Shows customer details, service, and date.
// No reply hint, no sign-off.
// ---------------------------------------------------------------------------
export function renderWorkerBookingNotification({
  serviceName,
  customerName,
  customerEmail,
  customerPhone,
  ...
  const title = cancelled
    ? (locale === "hu" ? "Foglalás lemondva" : "Booking Cancelled")
    : (locale === "hu" ? "Új foglalás" : "New booking");
  const serviceLabel = locale === "hu" ? "Szolgáltatás" : "Service";
  const dateLabel = locale === "hu" ? "Időpont" : "Date & time";
  const customerLabel = locale === "hu" ? "Ügyfél" : "Customer";
  const emailLabel = locale === "hu" ? "E-mail" : "Email";
  const phoneLabel = locale === "hu" ? "Telefon" : "Phone";

  const dateRange = startsAt && endsAt
    ? `${fmtDate(startsAt)}, ${fmtTime(startsAt)} – ${fmtTime(endsAt)}`
    : "";
  const subject = cancelled
    ? (locale === "hu" ? `Lemondott foglalás: ${serviceName || ""}` : `Booking cancelled: ${serviceName || ""}`)
    : (locale === "hu" ? `Új foglalás: ${serviceName || ""}` : `New booking: ${serviceName || ""}`);

  const detailRows = [
    serviceName ? `<tr><td style="padding:6px 0;font-size:13px;color:${BRAND.muted};">${esc(serviceLabel)}:</td><td style="padding:6px 0 6px 12px;font-size:13px;color:${BRAND.foreground};font-weight:600;">${esc(serviceName)}</td></tr>` : "",
    dateRange ? `<tr><td style="padding:6px 0;font-size:13px;color:${BRAND.muted};">${esc(dateLabel)}:</td><td style="padding:6px 0 6px 12px;font-size:13px;color:${BRAND.foreground};font-weight:600;">${esc(dateRange)}</td></tr>` : "",
    customerName ? `<tr><td style="padding:6px 0;font-size:13px;color:${BRAND.muted};">${esc(customerLabel)}:</td><td style="padding:6px 0 6px 12px;font-size:13px;color:${BRAND.foreground};font-weight:600;">${esc(customerName)}</td></tr>` : "",
    customerEmail ? `<tr><td style="padding:6px 0;font-size:13px;color:${BRAND.muted};">${esc(emailLabel)}:</td><td style="padding:6px 0 6px 12px;font-size:13px;color:${BRAND.foreground};font-weight:600;">${esc(customerEmail)}</td></tr>` : "",
    customerPhone ? `<tr><td style="padding:6px 0;font-size:13px;color:${BRAND.muted};">${esc(phoneLabel)}:</td><td style="padding:6px 0 6px 12px;font-size:13px;color:${BRAND.foreground};font-weight:600;">${esc(customerPhone)}</td></tr>` : "",
    comment ? `<tr><td style="padding:6px 0;font-size:13px;color:${BRAND.muted};">${esc(locale === "hu" ? "Megjegyzés" : "Comment")}:</td><td style="padding:6px 0 6px 12px;font-size:13px;color:${BRAND.foreground};font-weight:600;">${esc(comment)}</td></tr>` : "",
  ].filter(Boolean).join("");

  const bodyHtml = `<h1 style="margin:0 0 16px 0;font-size:24px;line-height:1.25;font-weight:700;letter-spacing:-0.015em;color:${BRAND.foreground};">${esc(title)}</h1>
${detailRows ? `<table role="presentation" cellpadding="0" cellspacing="0" border="0" style="margin:0 0 24px 0;border-collapse:collapse;">${detailRows}</table>` : ""}`;

  const html = layout({ locale, preheader: title, bodyHtml });

  const detailText = [
    serviceName ? `${serviceLabel}: ${serviceName}` : "",
    dateRange ? `${dateLabel}: ${dateRange}` : "",
    customerName ? `${customerLabel}: ${customerName}` : "",
    customerEmail ? `${emailLabel}: ${customerEmail}` : "",
    customerPhone ? `${phoneLabel}: ${customerPhone}` : "",
    comment ? `${locale === "hu" ? "Megjegyzés" : "Comment"}: ${comment}` : "",
  ].filter(Boolean).join("\n");
...
export function renderForgotPassword({ userName, resetLink }) {
  ...
  const subject = "Jelszó visszaállítása — Nexus";
...
export function renderInvite({ userName, inviteLink, isReinvite = false }) {
  ...
  const body1 = isReinvite
    ...
    : "Egy adminisztrátor létrehozott neked egy fiókot a Nexus-ban. Kattints az alábbi gombra, hogy beállítsd a jelszavad:";
...
// ---------------------------------------------------------------------------
// Template 5: bulk email (admin → all project customers)
//
// Admin-initiated bulk message sent to every reservation customer of a
// project. Uses the project's own branding (header = project name,
// footer = project customer_email) — identical shell to the submitter
// auto-reply. The body is freeform text provided by the admin; newlines
// are preserved as <br> tags so the admin can format the message from
// the textarea.
// ---------------------------------------------------------------------------
...
export function renderBulkEmail({
  ...
  // Convert newlines to <br> for HTML rendering, preserving the admin's
  // formatting from the textarea.
  const bodyHtml = `<div style="font-size:14px;line-height:1.7;color:${BRAND.foreground};white-space:pre-wrap;">${nl2br(body)}</div>
<p style="margin:24px 0 0 0;font-size:14px;line-height:1.5;color:${BRAND.muted};">Üdvözlettel,<br><strong style="color:${BRAND.foreground};">${esc(header.name || "a csapat")}</strong></p>`;
