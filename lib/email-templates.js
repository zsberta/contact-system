...
export function renderBookingAutoReply({
  ...
  const confirmed = locale === "hu" ? "Foglalás megerősítve!" : "Booking confirmed!";
  ...
  const subjectDate = startsAt
    ...
    : "";
  const statusWord = locale === "hu" ? "Megerősítve" : "Confirmed";
...
export function renderWorkerBookingNotification({
  ...
  const fmtTime = (iso) => {
    if (!iso) return "";
    const d = new Date(iso);
    return d.toLocaleTimeString(locale === "hu" ? "hu-HU" : "en-US", { hour: "2-digit", minute: "2-digit" });
  };

  const title = locale === "hu" ? "Új foglalás" : "New booking";
  const serviceLabel = locale === "hu" ? "Szolgáltatás" : "Service";
  const dateLabel = locale === "hu" ? "Időpont" : "Date & time";
  ...
  const subject = locale === "hu" ? `Új foglalás: ${serviceName || ""}` : `New booking: ${serviceName || ""}`;
