...
async function runJob(job) {
  ...
  const logSend = (ok, attempt, extraMeta = {}) => {
    logActivity({
      req: null,
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt += 1) {
    ...
    let ok = false;
    try {
      // Kill switch (EMAIL_SENDING=false): sendMail reports false without
      // a real failure — retrying is pointless, drop the job immediately.
      if (process.env.EMAIL_SENDING === "false") {
        // Kill switch: nothing is sent, so nothing is logged — a disabled
        // queue is not a delivery failure and must not pollute the audit
        // trail with ok=false rows.
        inFlight.delete(job);
        scheduleWorker();
        return;
      }
      // job.args already excludes the audit sidecar (split in enqueueMail).
  ...
  logSend(false, MAX_ATTEMPTS, lastErr ? { error: String(lastErr.message || lastErr) } : {});
  inFlight.delete(job);
  scheduleWorker();
}
...
export function enqueueMail(args) {
  ...
  // Optional audit sidecar (NOT passed to the SMTP transport): copied
  // onto the job so runJob can log one email.send SEND row per delivery.
  // Shape: { action?, kind?, entityType?, entityId?, entityLabel?,
  //          projectId?, actor? }.
  const { audit, ...mailArgs } = args;
  const job = {
    id: nextJobId,
    args: mailArgs,
