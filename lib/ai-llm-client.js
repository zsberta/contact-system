 ...
 * @param {string}   [opts.sessionId]   - Stable per-conversation id, sent as
 *                                        `x-opencode-session` (required by OpenCode Go)
 * @param {object}   [opts.extraHeaders]- Extra headers merged into the request
 */
export async function streamChat({
  ...
  maxTokens,
  timeoutMs = 60_000,
  // Stable per-conversation id. Required by some OpenAI-compatible
  // providers (e.g. OpenCode Go enforces `x-opencode-session` for routing
  // and prompt caching — see https://opencode.ai/docs/go/#where-can-i-use-it).
  // Harmless for providers that don't need it (plain OpenAI ignores it).
  sessionId,
  extraHeaders,
}) {
  ...
  // /chat/completions endpoint URL. Never double up the path when the full
  // endpoint is already configured. Other protocols (/responses, /messages)
  // are NOT chat-completions — fail loudly instead of sending the wrong
  // protocol and failing confusingly downstream.
  const normalizedBase = baseUrl.replace(/\/+$/, "");
  if (/(\/responses|\/messages)$/.test(normalizedBase)) {
    onError(new Error(
      "streamChat: this client speaks OpenAI chat-completions only; " +
      `baseUrl points at an unsupported endpoint (${normalizedBase}). ` +
      "Use a /chat/completions model endpoint instead."
    ));
    return;
  }
  const url = /\/chat\/completions$/.test(normalizedBase)
    ? normalizedBase
  ...
  try {
    const response = await fetch(url, {
      ...
      headers: {
        ...
        // Identify as a real client (some providers reject generic SDK agents).
        "User-Agent": "contact-system-ai-assistant/1.0",
        ...(sessionId ? { "x-opencode-session": String(sessionId) } : {}),
        ...(extraHeaders || {}),
      },
