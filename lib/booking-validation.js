...
// route (routes/reservations.js), so both run the EXACT same checks.
// ----------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Slot-alignment validation — anchored to the schedule window start, NOT
// midnight.  A schedule from 07:30 with slot=60 produces valid slots at
// 07:30, 08:30, 09:30 … which are NOT on midnight-aligned boundaries.
// ---------------------------------------------------------------------------

/** Convert a UTC ISO timestamp to local wall-clock minutes-from-midnight. */
function utcToLocalMinutesFromMidnight(utcIso, tz) {
...
/**
 * Check that both startsAt and endsAt fall on a slot boundary relative
 * to `scheduleWindowStartMin` (the local-time start of the applicable
 * schedule window, in minutes from midnight; 0 = midnight / no schedule).
 *
...
export function validateSlotAlignment(
  ...
  slot,
  tz,
  scheduleWindowStartMin = 0,
) {
  const startLocalMin = utcToLocalMinutesFromMidnight(startsAtIso, tz);
  const startOffset = startLocalMin - scheduleWindowStartMin;
  if (startOffset < 0 || startOffset % slot !== 0) {
    return {
      ok: false,
      error: `startsAt must align to ${slot}-minute slot boundary`,
    };
  }
  const endLocalMin = utcToLocalMinutesFromMidnight(endsAtIso, tz);
  const endOffset = endLocalMin - scheduleWindowStartMin;
  if (endOffset <= 0 || endOffset % slot !== 0) {
    return {
      ok: false,
 ...
 * `POST /api/reservations/:id/bookings` (admin).
 */
export async function validateBookingItem({
  ...
  // Service-level config wins when present: the admin create route used to
  // validate against the reservation-level grid only, letting operators
  // book off-grid slots the public embed would reject.
  const slotGrid = service && service.slot_duration_minutes !== null && service.slot_duration_minutes !== undefined
    ? service.slot_duration_minutes
    : reservation.slot_duration_minutes;
  const gridGranularity = (service && service.granularity) || reservation.granularity;
  if (
    slotGrid !== null &&
    slotGrid !== undefined &&
    gridGranularity !== "day"
    ...
    if (!alignResult.ok) return alignResult;
  }
