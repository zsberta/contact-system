...
// Embeddings
// ---------------------------------------------------------------------------
async function generateEmbeddings({ baseUrl, apiKey, chunks, model, sessionId }) {
    ...
    try {
      response = await fetch(url, {
        ...
        headers: {
          ...
          // Some OpenAI-compatible providers (e.g. OpenCode Go) require a
          // stable session id for routing — harmless elsewhere.
...
async function processDocument({ documentId, assistantId, filePath, fileType }) {
  ...
  try {
    ...
    const embeddings = await generateEmbeddings({ baseUrl, apiKey, chunks, model: embeddingModel, sessionId: `knowledge-assistant-${assistantId}` });
...
// ---------------------------------------------------------------------------
// Entry point — read task from CLI args (passed by ai-knowledge-processor.js)
// Usage: node ai-knowledge-worker.js <documentId> <assistantId> <filePath> <fileType>
// ---------------------------------------------------------------------------
const args = process.argv.slice(2);
if (args.length >= 4) {
  const msg = {
    documentId: Number(args[0]),
    assistantId: Number(args[1]),
    filePath: args[2],
    fileType: args[3],
  };
  console.error("[ai-knowledge-worker] received task:", JSON.stringify({ documentId: msg.documentId, fileType: msg.fileType }));
  processDocument(msg).catch((err) => {
    console.error("[ai-knowledge-worker] fatal:", err.message);
    process.exit(1);
  });
} else {
  console.error("[ai-knowledge-worker] no task args, exiting");
  process.exit(1);
}

// Catch unhandled errors
process.on("uncaughtException", (err) => {
  console.error("[ai-knowledge-worker] uncaught:", err.message);
  process.exit(1);
});
process.on("unhandledRejection", (err) => {
  console.error("[ai-knowledge-worker] unhandled rejection:", err?.message || err);
});
