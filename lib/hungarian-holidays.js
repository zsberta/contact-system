...
/**
 * Check if a specific date is a Hungarian public holiday.
 * Returns the holiday base key (e.g. "new_year") or null.
 * Easter-based holidays are computed dynamically.
 */
function isHungarianHoliday(year, month, day) {
  // Fixed holidays
  for (const h of FIXED_HOLIDAYS) {
  ...
  const easterMonday = addDays(easter.month, easter.day, +1);
  const whitMonday   = addDays(easter.month, easter.day, +50);
  if (goodFriday.month === month && goodFriday.day === day) return "good_friday";
  if (easterMonday.month === month && easterMonday.day === day) return "easter_monday";
  if (whitMonday.month === month && whitMonday.day === day) return "whit_monday";
  return null;
}
// ---------------------------------------------------------------------------
// Bridge rest days (pihenőnapok) per year — government-decreed days off that
// are NOT public holidays (e.g. 2026-08-21, the Friday after Aug 20).
//
// These CANNOT be computed: the naive "Tue→Mon / Thu→Fri bridge" rule would
// wrongly block days the decree left as workdays (e.g. 2026-04-30 and
// 2026-10-22). Extend the table yearly from the official decree; years
// absent below simply have no rest days. Consumers toggle them with the
// single generic REST_DAY_KEY, which recurs every year like other holidays.
// ---------------------------------------------------------------------------
const REST_DAY_KEY = "rest_day";

const REST_DAYS_BY_YEAR = {
  2026: [[8, 21], [12, 24]],
};

function getRestDaysForYear(year) {
  return (REST_DAYS_BY_YEAR[year] || []).map(([month, day]) => ({ month, day }));
}

function isRestDay(year, month, day) {
  return getRestDaysForYear(year).some((d) => d.month === month && d.day === day);
}

export { getHolidaysForYear, generateHolidayRows, isHungarianHoliday, FIXED_HOLIDAYS, isRestDay, getRestDaysForYear, REST_DAY_KEY };
