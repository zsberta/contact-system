 ...
 */
async function generateEmbeddings({ baseUrl, apiKey, chunks }) {
...
export async function processDocument({ documentId, assistantId, filePath, fileType }) {
  ...
  try {
    ...
    //    The api_key_enc is expected to be decrypted by the caller or
    //    stored as plaintext in development. For the MVP, we read the
    //    raw column and pass it through.
    ...
    const { base_url: baseUrl, api_key_enc: apiKey } = configRows[0];
    if (!apiKey) {
      throw new Error("No API key configured for this assistant");
    }

    // 4. Generate embeddings
    const embeddings = await generateEmbeddings({ baseUrl, apiKey, chunks });
    ...
    try {
      ...
      for (let i = 0; i < chunks.length; i++) {
        ...
        await client.query(
          ...
          [documentId, assistantId, i, chunks[i], embeddingLiteral, tokenCount],
        );
      }
