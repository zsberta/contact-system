...
export async function getScheduleWindowStartMin(
  ...
  // Find the matching window and return its start_time in minutes.
  for (const s of schedulesResult.rows) {
    let dayMatches = false;
    ...
    const schedStart = typeof s.start_time === "string" ? s.start_time.slice(0, 5) : s.start_time;
    const [h, m] = schedStart.split(":").map(Number);
    return h * 60 + m;
  }

  // Day has schedules but none matched → treat as closed (return 0 and let
  // the availability check reject the slot).
  return 0;
}
