...
import { isHungarianHoliday, isRestDay, REST_DAY_KEY } from "./hungarian-holidays.js";
...
export async function checkSlotAvailability(
  ...
  const holidayKey = isHungarianHoliday(localYear, localMonth, localDay)
    ?? (isRestDay(localYear, localMonth, localDay) ? REST_DAY_KEY : null);
  if (holidayKey) {
    if (serviceId) {
...
// ---------------------------------------------------------------------------
// getScheduleWindowStartMin — look up the applicable schedule window for a
// booking's start time and return its start offset (minutes from midnight,
// local time).  Returns 0 when no schedules exist (open 24/7), preserving
// the legacy midnight-anchor behaviour.
// ---------------------------------------------------------------------------

export async function getScheduleWindowStartMin(
  reservationId,
  serviceId,
  startsAtIso,
  tz,
  ...
  // Query schedules — service-level first, fallback to reservation-level.
  let schedulesResult;
  if (serviceId) {
    schedulesResult = await db.query(
      `SELECT frequency, day_of_week, day_of_month, start_time, end_time
  ...
  if (schedulesResult.rowCount === 0) return 0;

  // Compute local date components from the UTC start time.
  ...
  const localYear = Number(vals.year);
  const localMonth = Number(vals.month);
  const localDay = Number(vals.day);
  const dow = new Date(Date.UTC(localYear, localMonth - 1, localDay)).getUTCDay();
  const dom = localDay;

  // Compute local start time in minutes from midnight for containment check.
  const localHour = Number(vals.hour);
  const localMinute = Number(vals.minute);
  const startLocalMin = localHour * 60 + localMinute;

  // Find the schedule window that contains the requested start time.
  // When multiple windows exist for the same day (e.g. 09:30-10:30 and
  // 17:00-18:00), returning the first match's start time causes slot-
  // alignment validation to fail for times in later windows because the
  // offset from the wrong anchor is not a multiple of the slot duration.
  let firstMatchStart = null;
  for (const s of schedulesResult.rows) {
    let dayMatches = false;
    if (s.frequency === "daily") dayMatches = true;
    else if (s.frequency === "weekly") dayMatches = Number(s.day_of_week) === dow;
    else if (s.frequency === "monthly") dayMatches = Number(s.day_of_month) === dom;
    if (!dayMatches) continue;

    const schedStartStr = typeof s.start_time === "string" ? s.start_time.slice(0, 5) : s.start_time;
    const [sh, sm] = schedStartStr.split(":").map(Number);
    const schedStartMin = sh * 60 + sm;

    // Remember first matching day's start as fallback (closed case).
    if (firstMatchStart === null) firstMatchStart = schedStartMin;

    // Check containment: startLocalMin must fall inside [start, end).
    if (s.end_time) {
      const schedEndStr = typeof s.end_time === "string" ? s.end_time.slice(0, 5) : s.end_time;
      const [eh, em] = schedEndStr.split(":").map(Number);
      const schedEndMin = eh * 60 + em;
      if (startLocalMin >= schedStartMin && startLocalMin < schedEndMin) {
        return schedStartMin;
      }
    } else {
      // No end_time — treat as open-ended; any start on the day matches.
      return schedStartMin;
    }
  }

  // No window contained the start time (or no day matched) → return the
  // first matching day's start as fallback. The slot-alignment check will
  // reject the booking, which is correct — the time falls outside all
  // defined windows for this day.
  return firstMatchStart !== null ? firstMatchStart : 0;
}
...
//
// `ignoreLeadTime` — when true, only past slots are skipped (slot start must
// be in the future) and the service's `lead_time_minutes` preparation window
// is NOT applied. Used by the admin calendar manual-booking picker so an
// operator can quick-book just before a service starts. The public embed
// keeps lead-time filtering for customers.
// ---------------------------------------------------------------------------
...
export async function getServiceAvailability({
  ...
  // NOTE: no LIMIT — every overlapping range must load, otherwise additional
  // blocked windows silently stay bookable.
  ...
  for (let d = 0; d <= diffDays; d++) {
    ...
    for (const window of scheduleWindows) {
      ...
      for (let min = winStartMin; min + durationMin <= winEndMin; min += interval) {
        ...
        // Skip phantom slots — local wall times that don't exist because of
        // spring-forward DST (e.g. 02:30 on the transition day). The offset
        // resolver maps them to an instant that displays as a different
        // wall time, so the label would lie about the actual slot.
        if (formatTimeInTz(startsAt, tz) !== startTime) {
          continue;
        }
        ...
        // Admin manual booking passes `ignoreLeadTime`: then only past slots are
        // skipped (slot start must still be in the future) so an operator can
        // quick-book just before a service starts.
        ...
        const leadTimeMs = ignoreLeadTime ? 0 : (service.lead_time_minutes || 0) * 60000;
        ...
        const slotHolidayKey = isHungarianHoliday(holidayYear, holidayMonth, holidayDay)
          ?? (isRestDay(holidayYear, holidayMonth, holidayDay) ? REST_DAY_KEY : null);
...
function getUtcOffsetMinutes(localDateTimeStr, timezone) {
  ...
  // Render an instant as local wall-clock milliseconds (as if UTC).
  ...
  // Initial guess: offset the target would display when read as UTC.
  let offsetMin = (targetMs - localMs(targetMs)) / 60000;

  // Round-trip verification: the chosen instant must display back as the
  // requested local wall time. Without this, DST-ambiguous times (e.g.
  // 01:00 on the fall-back day) resolved to the wrong occurrence and
  // slot labels drifted an hour from the instants they represented.
  // Ambiguous times converge on one occurrence; nonexistent times (spring
  // forward) oscillate and are rejected by the caller's label check.
  for (let i = 0; i < 2; i++) {
