...
export async function createReservationBooking({
  ...
  try {
    ...
    // Duplicate booking check — email is the customer's unique identity
    // (phone is a plain, updatable contact field and must never block a
    // booking: e.g. family members sharing one landline with separate
    // emails can book the same class slot).
    if (contact?.email) {
      const dupResult = await client.query(
        `SELECT 1
         ...
         WHERE service_id = $1
           ...
           AND email = $4
         LIMIT 1`,
        [service.id, startsAtIso, endsAtIso, contact.email],
      );
    ...
    const insertResult = await client.query(
      `INSERT INTO reservation_bookings (
       ...
       RETURNING id, reservation_id, service_id, starts_at, ends_at,
                 ...
                 service_name_snapshot, duration_minutes_snapshot,
                 price_amount_snapshot, currency_snapshot, timezone,
                 status, source, locale, booking_token`,
      [
        reservation.id,
        service.id,
        startsAtIso,
