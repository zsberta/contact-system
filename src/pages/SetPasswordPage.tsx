// Public page: /set-password?token=<plain>
//
// Reached by clicking the invite link in the welcome email. The token
// is the only auth — we don't have (and don't want) a session here.
// On success we redirect to /login with a success toast.
//
// The page is intentionally minimal — there's no shared layout, no
// navigation, and the design mirrors Login.tsx so returning users
// recognise the brand.
...
const SetPasswordPage: React.FC = () => {
  ...
  const onSubmit = async (values) => {
    try {
      await setPassword({ token, newPassword: values.newPassword });
      showSuccess(t("auth:set_password_success"));
      navigate("/login");
    } catch (error) {
      const msg = error?.message || t("common:operation_failed", { error: "" });
      if (
        typeof msg === "string" &&
        ...
        <Card className="border-none shadow-lg pt-8 max-w-md w-full mx-3 z-10">
          ...
          <CardContent className="space-y-4">
            <p className="text-sm text-muted-foreground text-center">
              {t("auth:set_password_invalid_token")}
            </p>
  ...
  return (
    <div className="min-h-screen flex items-center justify-center relative overflow-hidden">
      <SEO
        fallbackTitle={t("auth:set_password_title")}
        fallbackDescription={t("auth:set_password_description")}
      />
      <ParticleBackground />
